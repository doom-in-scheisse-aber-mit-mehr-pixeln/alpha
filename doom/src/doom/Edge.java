...
//Please put all of these shit (and these shit from gradients) in one list (please do it!!!(now I haven't enough time))
public class Edge
{
	private float m_x;
	...
	{
		m_yStart = (int)Math.ceil(minYVert.Get(1));
		m_yEnd = (int)Math.ceil(maxYVert.Get(1));

		float yDist = maxYVert.Get(1) - minYVert.Get(1);
		float xDist = maxYVert.Get(0) - minYVert.Get(0);

		float yPrestep = m_yStart - minYVert.Get(1);
		m_xStep = (float)xDist/(float)yDist;
		m_x = minYVert.Get(0) + yPrestep * m_xStep;
		float xPrestep = m_x - minYVert.Get(0);
