...
public class Player
{
	...
	public void Update(Display display, float delta)
	{
		...
		System.out.println(GetTransform().GetRot().GetY());
		...
		Rotate(Y_AXIS, SensitivityX*(input.GetMouseXOnScreen()-m_xMouseStartPoint));
		Rotate(GetTransform().GetRot().GetRight(), SensitivityY*(input.GetMouseYOnScreen()-m_yMouseStartPoint));
		try {
            robot = new Robot();
		...
		//}
		m_xMouseStartPoint=display.getWidth() / 2;//input.GetMouseX();
		m_yMouseStartPoint=display.getHeight() / 2;//input.GetMouseY();
