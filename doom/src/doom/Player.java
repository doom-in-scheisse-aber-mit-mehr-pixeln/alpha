...
public class Player extends Thread
{
	...
	public void run()
	{
		...
		while(true) {
			...
			if(shootingTime<=0&&input.GetMouse(1)&&weapon==1) {shootingTime=0.5f;shooting=true;}
	...
	public boolean getShooting() {
		boolean toReturn=shooting;
		shooting=false;
		return toReturn;
	}
	
	public PlayerInformation getPlayerInfo(PlayerInformation playerInfo, boolean a) {
		playerInfo.setHealth((byte)health);
		playerInfo.setPos(m_playerPos.Mul(-1));
		playerInfo.setRot(GetTransform().GetRot());
		if(a)
			playerInfo.setBullet(GetTransform().GetPos().Add(GetTransform().GetRot().GetForward().Mul(1000f-999*weapon)));
		else
			playerInfo.setBullet(playerInfo.getPos());
		
		return playerInfo;
	}
	
	public Vector4f getBullet(boolean shooting) {
		Vector4f shot=m_playerPos;
		if(shooting)
			shot=GetTransform().GetPos().Add(GetTransform().GetRot().GetForward().Mul(1000f-999*weapon));
		return shot;
	}
